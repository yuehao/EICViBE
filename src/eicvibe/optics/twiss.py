...
class TwissData(PhysicsBaseModel):
    ...
    element_names: Optional[list] = Field(default=None, description="Names of elements at each s position")
    element_types: Optional[list] = Field(default=None, description="Types of elements at each s position")
    
    # ========== Computation Metadata ==========
    engine_name: Optional[str] = Field(default=None, description="Name of engine that computed this data")
    computation_method: Optional[str] = Field(default=None, description="Method used (e.g., '4d', '6d')")
    timestamp: Optional[str] = Field(default=None, description="When this data was computed")
    ...
    def summary(self) -> str:
        ...
        lines.append(f"\nBeta Functions:")
        lines.append(f"  βx: min = {np.min(self.beta_x):.2f} m, max = {np.max(self.beta_x):.2f} m")
        lines.append(f"  βy: min = {np.min(self.beta_y):.2f} m, max = {np.max(self.beta_y):.2f} m")
        
        if self.dx is not None:
            lines.append(f"\nDispersion:")
            lines.append(f"  Dx: min = {np.min(self.dx):.3f} m, max = {np.max(self.dx):.3f} m")
