...
from ..models.parameter_groups import MagneticMultipoleP, BendP, RFP, ApertureP
...
class XSuiteSimulationEngine(BaseSimulationEngine):
    ...
    def __init__(self, name: str = "xsuite", config: Optional[Dict[str, Any]] = None):
        ...
        if not XSUITE_AVAILABLE:
            raise ImportError(
                "XSuite is required for this engine. Install with: pip install xsuite"
            )

        # Create engine configuration
        from .base import EngineConfiguration

        engine_config = EngineConfiguration(name=name, **(config or {}))
    ...
    def convert_lattice(
        ...
        slice_config: Optional[Dict[str, int]] = None,
        ...
        Args:
            ...
            num_slices: Number of uniform slices for all thick elements (deprecated, use slice_config)
            slice_config: Dictionary mapping element types to number of slices
                         Default: {'Bend': 10, 'RBend': 10, 'Quadrupole': 10,
                                   'Sextupole': 5, 'Octupole': 5, 'Kicker': 1}
        ...
        """
        try:
            # Initialize slice configuration with defaults
            if slice_config is None:
                slice_config = {
                    ...
                    "Sextupole": 5,
                    "Octupole": 5,
                    "Kicker": 1,  # Correctors should not be sliced
                }

            # Legacy support: if num_slices is provided, use it as default for all elements
            if num_slices is not None:
                logger.warning(
                    "num_slices parameter is deprecated, use slice_config instead"
                )
                default_slices = num_slices
            else:
                default_slices = 10  # Fallback default

            # Store BPM configuration for use during element conversion
            self._bpm_config = {
                "num_turns": bpm_num_turns,
                "frev": bpm_frev,
                "sampling_frequency": bpm_sampling_frequency
                if bpm_sampling_frequency is not None
                else bpm_frev,
            }
            ...
            for element in elements_list:
                ...
                if xsuite_element is not None:
                    ...
                    if element.type == "Monitor" and element.length > 0:
                        ...
                        logger.debug(
                            f"Added drift of {element.length:.6f} m after Monitor {element.name}"
                        )
                    ...
                    if element.type == "RFCavity" and element.length > 0:
                        ...
                        element_names.append(f"{element.name}_drift")
                        logger.debug(
                            f"Added drift of {element.length:.6f} m after RFCavity {element.name}"
                        )

            # Create XTrack line
            line = xt.Line(elements=elements, element_names=element_names)

            # Apply element-type-specific slicing based on slice_config
            # Build a map from element name → EICViBE element type (only for elements in the line)
            element_names_set = set(element_names)
            elem_type_map = {
                elem.name: elem.type
                for elem in elements_list
                if elem.name in element_names_set
            }

            # Group elements by desired slice count from slice_config
            slice_groups: dict = {}   # n_slices -> [element_name, ...]
            no_slice_names: list = []  # elements explicitly NOT to be sliced

            for elem_name, elem_type in elem_type_map.items():
                n_slices = slice_config.get(elem_type, None)
                if n_slices is None:
                    # Type not in config: leave to XSuite default (no explicit strategy)
                    continue
                elif n_slices <= 1:
                    no_slice_names.append(elem_name)
                else:
                    slice_groups.setdefault(n_slices, []).append(elem_name)

            # Build XSuite slicing strategies
            # xt.Strategy uses name= as a regex pattern (exact=False by default)
            slicing_strategies = []

            def _names_to_regex(names: list) -> str:
                """Build a regex that exactly matches any of the given element names."""
                import re
                escaped = [re.escape(n) for n in names]
                return "^(" + "|".join(escaped) + ")$"

            # Strategies for elements to be sliced (highest slice count first)
            for n_slices in sorted(slice_groups.keys(), reverse=True):
                names = slice_groups[n_slices]
                if names:
                    pattern = _names_to_regex(names)
                    slicing_strategies.append(
                        xt.Strategy(slicing=xt.Teapot(n_slices), name=pattern)
                    )
                    logger.debug(
                        f"Slicing strategy: {n_slices} slices for {len(names)} elements"
                    )

            # Explicit no-slice strategy prevents XSuite from applying its own default
            if no_slice_names:
                pattern = _names_to_regex(no_slice_names)
                slicing_strategies.append(
                    xt.Strategy(slicing=None, name=pattern)
                )
                logger.debug(
                    f"No-slice strategy for {len(no_slice_names)} elements: {no_slice_names[:5]}"
                )

            if slicing_strategies:
                line.slice_thick_elements(slicing_strategies)
                total_sliced = sum(len(v) for v in slice_groups.values())
                logger.info(
                    f"Applied element-specific slicing: {total_sliced} elements sliced, "
                    f"{len(no_slice_names)} explicitly not sliced"
                )
            else:
                logger.info("No slicing strategies applied")
            ...
            if reference_energy is not None:
                ...
                if reference_species.lower() == "proton":
                    ...
                    q0 = 1
                    logger.warning(
                        "Using proton mass for 'ion' species. Override if needed."
                    )
                else:
                ...
                line.particle_ref = xp.Particles(
                    p0c=reference_energy, q0=q0, mass0=mass0
                )
                logger.info(
                    f"Set reference particle: {reference_species} at {reference_energy / 1e9:.2f} GeV"
                )
            ...
            logger.info(
                f"Converted lattice to XTrack Line with {len(elements)} elements (sliced)"
            )
            self._line = line
            return line
    ...
    def _convert_element(self, element: Element) -> Optional[Any]:
        ...
        if xsuite_type == "BeamPositionMonitor":
            ...
            if bpm_config.get("num_turns") is not None:
                ...
                params["sampling_frequency"] = bpm_config.get(
                    "sampling_frequency", params["frev"]
                )
                logger.debug(
                    f"Creating BPM {element.name} with stop_at_turn={params['stop_at_turn']}, frev={params['frev']:.2e}"
                )
        ...
        if xsuite_type == "Multipole":
            ...
            if not knl and not ksl:
                params["knl"] = [0.0]
                logger.debug(
                    f"Multipole {element.name} has no magnetic strengths - creating zero-strength element"
                )
            elif not knl:
            ...
            logger.debug(
                f"Created Multipole {element.name} with length={params.get('length', 0):.3f}, isthick={params.get('isthick', False)}, knl={params.get('knl', [])}, ksl={params.get('ksl', [])}"
            )
        ...
        if xsuite_type == "Cavity":
            ...
            if not params:
                ...
                logger.warning(
                    f"  Element has parameter groups: {element.get_all_parameters() if hasattr(element, 'get_all_parameters') else 'N/A'}"
                )

        # Create XSuite element
        logger.debug(
            f"Attempting to create {xsuite_type} for {element.name} with params: {params}"
        )
        try:
            xsuite_element = xsuite_class(**params)
            logger.debug(
                f"✓ Successfully converted {element.name} ({element_type}) to {xsuite_type}"
            )
    ...
    def create_particles(self, distribution: ParticleDistribution) -> Any:
        ...
        """
        try:
            # Determine species from line's particle_ref when available
            if self._line is not None and self._line.particle_ref is not None:
                _ref = self._line.particle_ref
                _q0 = float(_ref.q0)
                _mass0 = float(_ref.mass0)
            else:
                # Default to proton when no line is loaded yet
                _q0 = 1
                _mass0 = xp.PROTON_MASS_EV
            ...
            if dtype_str == "gaussian":
                ...
                if distribution.x_std is not None and distribution.px_std is not None:
                    ...
                    sigma_y = (
                        distribution.y_std if distribution.y_std is not None else 0.0
                    )
                    sigma_py = (
                        distribution.py_std if distribution.py_std is not None else 0.0
                    )
                    ...
                    particles.px = (
                        np.random.randn(distribution.num_particles) * sigma_px
                    )
                    ...
                    particles.py = (
                        np.random.randn(distribution.num_particles) * sigma_py
                    )
                    if distribution.py_mean is not None:
                        particles.py += distribution.py_mean

                elif (
                    distribution.emittance_x is not None
                    and distribution.beta_x is not None
                ):
                    ...
                    if distribution.alpha_x is not None:
                        particles.px -= (
                            distribution.alpha_x * particles.x / distribution.beta_x
                        )
                    if distribution.alpha_y is not None:
                        particles.py -= (
                            distribution.alpha_y * particles.y / distribution.beta_y
                        )
                    ...
                    raise ValueError(
                        "GAUSSIAN distribution requires either (x_std, px_std, y_std, py_std) or (emittance_x, emittance_y, beta_x, beta_y)"
                    )

                # Longitudinal distribution
                if distribution.delta_std is not None and distribution.delta_std > 0:
                    particles.delta = (
                        np.random.randn(distribution.num_particles)
                        * distribution.delta_std
                    )
                ...
                if distribution.zeta_std is not None and distribution.zeta_std > 0:
                    particles.zeta = (
                        np.random.randn(distribution.num_particles)
                        * distribution.zeta_std
                    )
                    if distribution.zeta_mean is not None:
                        particles.zeta += distribution.zeta_mean

            elif dtype_str == "uniform":
                # Uniform distribution over [min, max] ranges
                def _uniform_range(lo, hi, n):
                    if lo is None or hi is None:
                        return np.zeros(n)
                    return np.random.uniform(lo, hi, n)

                particles.x = _uniform_range(distribution.x_min, distribution.x_max, distribution.num_particles)
                particles.px = _uniform_range(distribution.px_min, distribution.px_max, distribution.num_particles)
                particles.y = _uniform_range(distribution.y_min, distribution.y_max, distribution.num_particles)
                particles.py = _uniform_range(distribution.py_min, distribution.py_max, distribution.num_particles)
                particles.zeta = _uniform_range(distribution.zeta_min, distribution.zeta_max, distribution.num_particles)
                particles.delta = _uniform_range(distribution.delta_min, distribution.delta_max, distribution.num_particles)

            elif dtype_str == "custom":
                # User-supplied coordinate arrays
                if distribution.custom_particles is None:
                    raise ValueError("CUSTOM distribution requires 'custom_particles' to be set")
                cp = distribution.custom_particles
                n = distribution.num_particles
                particles.x = np.asarray(cp.get("x", np.zeros(n)))
                particles.px = np.asarray(cp.get("px", np.zeros(n)))
                particles.y = np.asarray(cp.get("y", np.zeros(n)))
                particles.py = np.asarray(cp.get("py", np.zeros(n)))
                particles.zeta = np.asarray(cp.get("zeta", np.zeros(n)))
                particles.delta = np.asarray(cp.get("delta", np.zeros(n)))

            elif dtype_str == "matched":
                # 'matched' distributions must go through create_matched_particles() directly.
                # Calling it from here would cause infinite recursion via the fallback path.
                raise ValueError(
                    "Use engine.create_matched_particles() for MATCHED distributions "
                    "instead of create_particles()."
                )

            else:
                raise ValueError(
                    f"Unsupported distribution type: '{dtype_str}'. "
                    "Expected 'gaussian', 'uniform', 'custom', or 'matched'."
                )

            logger.info(
                f"Created {distribution.num_particles} particles with p0c={distribution.energy:.3e} eV"
            )
            logger.info(
                f"  σ_x={np.std(particles.x):.3e} m, σ_y={np.std(particles.y):.3e} m"
            )
    ...
    def create_matched_particles(
        ...
        if self._line is None:
            raise TrackingError(
                "Line must be initialized before creating matched particles"
            )
        ...
        try:
            ...
            if nemitt_x is None:
                ...
                logger.info(
                    f"  ε_x: {distribution.emittance_x:.3e} m → {nemitt_x:.3e} m"
                )
                logger.info(
                    f"  ε_y: {distribution.emittance_y:.3e} m → {nemitt_y:.3e} m"
                )

            # Determine creation point
            at_element = element_name if element_name else 0

            # Build transverse-matched particles using line.build_particles.
            # This uses normalized Gaussian coordinates and the 4D Twiss W-matrix,
            # completely bypassing RF bucket characterization (which fails / warns
            # when the lattice has no stable RF bucket configured).
            n = distribution.num_particles
            sigma_z = (
                distribution.zeta_std
                if distribution.zeta_std and distribution.zeta_std > 0
                else 0.0
            )
            delta_std = (
                distribution.delta_std
            ...
            particles = self._line.build_particles(
                x_norm=np.random.randn(n),
                px_norm=np.random.randn(n),
                y_norm=np.random.randn(n),
                py_norm=np.random.randn(n),
                zeta=np.random.randn(n) * sigma_z,
                delta=np.random.randn(n) * delta_std,
                nemitt_x=nemitt_x,
                nemitt_y=nemitt_y,
                method='4d',
            ...
            if use_lattice_twiss:
                twiss = self._line.twiss(
                    method='4d',
                    at_elements=[at_element] if element_name else [0]
                )
                beta_x_lattice = (
                    twiss.betx[0] if hasattr(twiss.betx, "__getitem__") else twiss.betx
                )
                beta_y_lattice = (
                    twiss.bety[0] if hasattr(twiss.bety, "__getitem__") else twiss.bety
                )
                logger.info(
                    f"Matched particles to lattice Twiss at {element_name or 'start'}:"
                )
                logger.info(
                    f"  β_x = {beta_x_lattice:.3f} m, β_y = {beta_y_lattice:.3f} m"
                )
            ...
            logger.info(
                f"  <x·px> = {np.mean(particles.x * particles.px):.3e} (coupling indicator)"
            )
            ...
            raise TrackingError(f"Failed to create matched particles: {e}")
    ...
    def get_twiss(
        self, converted_lattice: Any, energy: float, mode: SimulationMode, **kwargs
    ) -> TwissData:
        """
    ...
    def update_element_parameter(
        self, element_name: str, parameter_group: str, parameter_name: str, value: float
    ) -> bool:
        """
    ...
    def get_element_parameter(
        self, element_name: str, parameter_group: str, parameter_name: str
    ) -> Optional[float]:
        """
    ...
    def track(self, particles: Any, parameters: TrackingParameters) -> TrackingResults:
        ...
        try:
            ...
            results = TrackingResults(
                ...
                final_coordinates={
                    "x": particles.x.copy()
                    if hasattr(particles.x, "copy")
                    else np.array(particles.x),
                    "px": particles.px.copy()
                    if hasattr(particles.px, "copy")
                    else np.array(particles.px),
                    "y": particles.y.copy()
                    if hasattr(particles.y, "copy")
                    else np.array(particles.y),
                    "py": particles.py.copy()
                    if hasattr(particles.py, "copy")
                    else np.array(particles.py),
                    "zeta": particles.zeta.copy()
                    if hasattr(particles.zeta, "copy")
                    else np.array(particles.zeta),
                    "delta": particles.delta.copy()
                    if hasattr(particles.delta, "copy")
                    else np.array(particles.delta),
                    "state": particles.state.copy()
                    if hasattr(particles.state, "copy")
                    else np.array(particles.state),
                },
            )
    ...
    def get_closed_orbit(
        self, converted_lattice: Any, energy: float
    ) -> Dict[str, float]:
        """
    ...
    def get_bpm_readings(
        self, monitor_elements: List[Element]
    ) -> Dict[str, BeamPositionData]:
        """
    ...
    def configure_bpm_tracking(
        ...
        for name, elem in zip(self._line.element_names, self._line.elements):
            if isinstance(elem, xt.BeamPositionMonitor):
                ...
                bpm_count += 1
                logger.debug(
                    f"Configured {name}: num_turns={num_turns}, frev={frev:.2e} Hz, sampling={sampling_frequency:.2e} Hz"
                )
